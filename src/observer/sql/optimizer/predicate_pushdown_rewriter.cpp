...
RC PredicatePushdownRewriter::rewrite(unique_ptr<LogicalOperator> &oper, bool &change_made)
{
  ...
  unique_ptr<Expression> &pred_expr = pred_oper->expressions().front();
  /* if (pred_expr->type() == ExprType::COMPARISON) {
   * } else {  // pred_expr->type() == ExprType::CONJUNCTION
   * } */

  table_get_oper->set_predicate(std::move(pred_expr));

  LOG_TRACE("all expressions of predicate operator were pushdown to table get operator, then make a fake one");
  Value value((bool)true);
  pred_expr = unique_ptr<Expression>(new ValueExpr(value));
  change_made = true;
...
RC PredicatePushdownRewriter::get_exprs_can_pushdown(
    unique_ptr<Expression> &expr, unique_ptr<Expression> &pushdown_expr)
{
  return RC::SUCCESS;
}

bool PredicatePushdownRewriter::is_empty_predicate(unique_ptr<Expression> &expr) { return true; }
 ...
 *     table_get_oper->set_predicate(std::move(pushdown_exprs.front()));
 *   }
 *   return rc;
 * } */
